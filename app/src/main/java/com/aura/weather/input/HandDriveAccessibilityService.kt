...
class HandDriveAccessibilityService : AccessibilityService() {
    ...
    private fun screenSize(): Pair<Float, Float> {
        ...
        val w = if (metrics.widthPixels > 0) metrics.widthPixels.toFloat() else resources.displayMetrics.widthPixels.toFloat()
        val h = if (metrics.heightPixels > 0) metrics.heightPixels.toFloat() else resources.displayMetrics.heightPixels.toFloat()
